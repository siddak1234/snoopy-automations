...
byte for byte, so this copy is the contract as the platform actually enforces it.
...
`automation-manifest.json` with one documented deviation: `service.origin` may be
plain `http` on a `*.internal` host, which the platform's own `validateManifest`
permits and the emitted schema does not. That disagreement is the platform's to
resolve and is filed with it.
